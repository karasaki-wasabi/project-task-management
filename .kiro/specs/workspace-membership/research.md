...
## 13. 実装完了後の文書追記（2026-08-09）

実装と検証で判明した、後続・運用向けの制約と文書修正点。

### soft-delete とメンバー一意制約
- `WorkspaceMember` の `@@unique([workspaceId, userId])` は論理削除行も対象
- 本仕様はメンバー個別削除が Non-Goal のため問題にならない
- 将来「脱退／再追加」を論理削除のまま入れると、再追加が P2002 で失敗する
- 後続では部分一意（active 行のみ）や論理削除行の復元方針を設計すること

### 空状態コピー
- モック／初期実装にあった「案件やタスクを共有」表現は、本仕様スコープ外（`workspace-resource-scope`）のため、可視境界／メンバー追加の表現に差し替えた

### 権限ステータスの整理
- 閲覧・検索・追加・設定の非メンバーは 403
- 削除の非所属は 404、非作成者メンバーは 403（一律 403 ではない）
