...
### 採用したレイアウト方針
- **案件バー**: 日セルごとに独立したセグメントではなく、**週単位のレーン割り当て**(区間スケジューリング、最大3レーン)で重なりを検出して詰め、絶対配置のオーバーレイ(`grid-column`のスパンで日付範囲を表現)として描画する。レーンに収まらない案件は週の右端に「他N件」チップとしてまとめ、クリックでその週の全案件を一覧表示するポップアップを開く
