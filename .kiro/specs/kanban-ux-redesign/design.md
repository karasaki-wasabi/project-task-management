...
- フォーカス領域・開発段階未設定一覧から開発段階別ボードへの「ドラッグ&ドロップ以外」の新しい移動手段(モーダル編集等)の追加
...
|-----------|--------------|--------|---------------|---------------------------|-----------|
| `kanban/index.vue` | Frontend/kanban | ページ全体のデータ取得・導出計算(担当者集計、バックログ抽出、子タスク進捗)・担当者フィルタ状態の管理 | 1, 2, 3, 4, 6 | useApiClient (P0), TaskCard (P0), AssigneeFocusTray/TeamWorkloadSummary/UnassignedBacklogPanel (P0) | State |
| `AssigneeFocusTray` | Frontend/kanban | 選択担当者の未完了タスクを固定領域に表示 | 1.1-1.5 | TaskCard (P0) | - |
...
| `TaskCard` | Frontend/kanban | 状態・優先度・進捗表示規約を一元化したカード/リスト行 | 5.1-5.5, 6.1-6.2 | - | - |
...
| Requirements | 1.1, 1.2, 1.3, 1.5, 2.1, 3.1, 3.6, 4.1, 4.2, 4.3, 4.4, 6.1, 6.2 |
...
| Field | Detail |
|-------|--------|
| Intent | 選択中の担当者の未完了タスクを、開発段階の有無によらず固定高さの領域に表示する |
