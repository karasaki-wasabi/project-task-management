...
- `/kiro-validate-impl`(タスク10-15完了後の全体検証): タスク14.1のDatePicker組み込みにより、`cases.spec.ts`以外の既存E2E(`kanban-case-link.spec.ts`2件、`dashboard.spec.ts`1件)が旧テキスト入力への`.fill("終了日", ...)`を呼んだままになっており失敗していた(タスク15.2ではcases.spec.ts自身の回帰にしか対応していなかったための見落とし)。日付に依存しないシナリオ(`kanban-case-link.spec.ts`)は未入力のまま登録するよう変更し、過去の終了日が必要なシナリオ(`dashboard.spec.ts`)は`cases.spec.ts`と同じ`setDateViaPicker`ヘルパーを複製して対応した。あわせて、ユーザーからの見た目・挙動フィードバックでクイック選択肢を今日のみに縮小した際、`requirements.md`(10.2)・`design.md`(DatePicker仕様)に残っていた「明日/1週間後/月末/来月1日」の記述も実装に合わせて更新した。
- E2Eを複数ファイル同時実行すると(例: `playwright test kanban-case-link.spec.ts dashboard.spec.ts cases.spec.ts`)Playwrightが複数ワーカーに分散し、単一の開発用DB/フロントエンドサーバーへの同時アクセスが原因と見られるタイムアウトが散発する(本スペック変更前から共有されている開発DBの性質によるもの)。ファイル単体実行(`playwright test cases.spec.ts`等、このプロジェクトで一貫して使ってきた検証方法)では毎回安定して成功することを複数回確認済み。
