...
本機能は、既存の「納品(Delivery)」機能を「案件(Case)」へ全面改称し、開始日・終了日・完了フラグを追加した上で、ポップアップ形式の登録・編集UI、未割当タスクの選択関連付け、およびカンバン画面のタスク詳細ポップアップからの案件関連付けを提供する。
...
- 案件に開始日・終了日・完了フラグを追加し、ポップアップ形式で登録・編集できるようにする
...
### This Spec Owns
- 案件(Case)のデータモデル・API・UI一式(旧`deliveries`モジュールの全面改称・拡張。名称・フィールド追加・汎用更新APIすべてを含む)
- タスク⇄案件の関連付け(`caseId`/`isRequiredForCase`)を設定する2つの入口: 案件登録ポップアップ(新規)とカンバンのタスク詳細ポップアップ拡張(既存モーダルへの追加)
- 案件の期限超過判定ロジック(終了日+完了フラグ基準への置き換え)
...
- 新規要素: `CaseFormModal`(登録)、`CaseDetailModal`(詳細/編集)の2コンポーネント。既存の`TaskDetailModal`は破壊的変更なしで1ブロック追加
...
│   ├── case.types.ts               # Case, CreateCaseInput, UpdateCaseInput, CaseProgress
│   ├── case.repository.ts          # Prisma経由データアクセス(name/startDate/endDate/isCompleted, detach-on-delete維持)
│   ├── case.service.ts             # 作成/汎用更新/進捗算出(終了日+完了フラグ基準)/削除、recurrence通知
...
frontend/components/
├── cases/
│   ├── CaseFormModal.vue           # 新規: 登録ポップアップ(案A、name/startDate/endDate + 未割当タスク選択+必須トグル)
│   └── CaseDetailModal.vue         # 新規: 詳細/編集ポップアップ(view: 関連タスク進捗表示 / edit: name/startDate/endDate/完了トグル)
├── kanban/
...
| 2.1–2.5 | 案件の登録 | CaseFormModal, CaseService | `POST /api/cases` | 案件登録フロー |
...
| 5.1–5.4 | 案件情報の編集 | CaseDetailModal, CaseService | `PATCH /api/cases/:id` | - |
| 6.1–6.3 | 期限超過判定 | CaseService.getProgress | `GET /api/cases/:id/progress` | - |
...
|---|---|---|---|---|---|
| CaseService | Backend/cases | 案件のCRUD・進捗算出・期限超過判定 | 2, 5, 6, 7, 8 | RecurrenceService (P1) | Service, API |
| TasksService(拡張) | Backend/tasks | `caseId`/`isRequiredForCase`の設定・未割当フィルタ | 3, 4 | - | Service, API |
| RecurrenceService(拡張) | Backend/recurrence | 案件連動テンプレートの生成トリガー名・参照フィールド追従 | (adjacent) | CaseService (P0 inbound) | Service |
| CaseFormModal | Frontend/cases | 案件登録ポップアップ(案A) | 2, 3, 9 | useApiClient (P0) | State |
| CaseDetailModal | Frontend/cases | 案件詳細/編集ポップアップ(削除操作を含む) | 5, 6, 7, 8, 9 | useApiClient (P0) | State |
...
| Field | Detail |
|-------|--------|
| Intent | 案件の作成・汎用更新・進捗算出・削除、および終了日+完了フラグ基準の期限超過判定 |
| Requirements | 2.1, 2.2, 2.3, 2.4, 2.5, 5.1, 5.2, 5.3, 5.4, 6.1, 6.2, 6.3, 7.1, 7.2, 8.1, 8.2 |

**Responsibilities & Constraints**
- `name`は空文字不可(既存`delivery.service.ts`のtrim検証を継承)
- `startDate`は省略可(nullable)、`endDate`は必須。両方指定時、`startDate > endDate`なら`badRequest`
...
**Dependencies**
- Outbound: RecurrenceService — `onCaseCreated`(作成時)/`onCaseEndDateChanged`(終了日変更時)を呼び出す (P1)。既存の`delivery.service.ts`同様、素の`await`(try/catchなし)で呼び出す。呼び出し先が例外を投げた場合、`Case`行自体は既に作成/更新済みのまま`create`/`update`全体が例外で終わる、既存の暗黙的挙動をそのまま維持する(新たなエラーハンドリングは追加しない)
...
interface CaseProgress {
  ...
  isOverdueWithIncomplete: boolean; // = !isCompleted && endDate < now && requiredIncomplete > 0
}
...
```
- Preconditions: `create`/`update`で`startDate`と`endDate`が両方与えられる場合、`startDate <= endDate`
- Postconditions: `create`は`isCompleted=false`固定。`update`の`isCompleted`変更は他フィールドと無関係に反映される
- Invariants: `isOverdueWithIncomplete`は`isCompleted=true`のとき常に`false`

##### API Contract
| Method | Endpoint | Request | Response | Errors |
|--------|----------|---------|----------|--------|
| POST | /api/cases | `{ name, startDate?, endDate }` | 201 `Case` | 400 |
| PATCH | /api/cases/:id | `{ name?, startDate?, endDate?, isCompleted? }` | 200 `Case` | 400, 404 |
...
- Validation: Zodスキーマで`startDate`/`endDate`を`z.coerce.date()`、`isCompleted`を`z.boolean().optional()`とする
...
**Implementation Notes**
- Integration: `CaseService.create`/`CaseService.update`(`endDate`変更時のみ)から呼び出す。呼び出し契約(戻り値・エラー伝播)は既存と同一
...
| Requirements | 2.1, 2.2, 2.3, 2.4, 2.5, 3.1, 3.2, 3.3, 3.4, 3.5, 3.6, 3.7, 9.2 |
...
- `startDate > endDate`の場合、送信前にクライアント側でエラー表示し送信しない(バックエンドの400も併せて捕捉)
...
- Outbound: useApiClient — `createCase`, `listTasks`, `updateTask` (P0)
...
| Requirements | 5.1, 5.2, 5.3, 5.4, 6.3, 7.2, 8.1, 8.2, 9.1, 9.2 |

**Responsibilities & Constraints**
- `TaskDetailModal`と同じ「既定は閲覧モード→編集ボタン→編集モード→保存→閲覧モードに戻る」フローを踏襲
- 閲覧モード: 開始日・終了日・完了状態・必須タスク進捗(件数+進捗バー)、関連タスクの簡易リスト(完了/未着手アイコン+必須バッジ、mockup 1f)
- 編集モード: name/startDate/endDateの入力欄、「この案件を完了にする」トグル(必須タスクの完了状況とは無関係に切り替え可能、5.4)
...
- Outbound: useApiClient — `getCase`または`listCases`結果の再利用、`updateCase`, `getCaseProgress`, `deleteCase` (P0)
...
| `deliveries.due_date` | `cases.end_date` |
...
**マイグレーション手順**:
1. `schema.prisma`を上記の通り更新(`@@map`/`@map`のマッピング先も変更)
2. `backend/src/prisma/migrations/`配下の既存マイグレーションディレクトリを全て削除
3. 開発DB(Docker Compose `mysql`)のデータをリセット(volumeの削除等)
4. `docker compose exec backend npx prisma migrate dev --name init_domain_schema`で、リネーム後のスキーマから単一の初期マイグレーションを生成・適用する
...
- **400 (バリデーション)**: `startDate > endDate`、案件名の空入力 → フォームのフィールド近傍にエラー表示
...
- **Unit Tests (backend)**
  - `case.service.ts`: `startDate > endDate`で400、`create`は`isCompleted=false`固定、`getProgress`は`isCompleted=true`のとき`isOverdueWithIncomplete=false`(終了日超過でも)
  - `task.repository.ts`: `unassignedCase: true`で`caseId IS NULL`のタスクのみ返る
- **Integration Tests (backend, 実HTTP経路)**
  - `POST /api/cases` → タスク未選択でも201、`PATCH /api/cases/:id`でname/startDate/endDate/isCompletedを独立に更新できる
  - `DELETE /api/cases/:id`後、紐づく`Task`/`Event`の`caseId`が`null`になっている(既存挙動のリネーム後回帰確認)
  - `recurrence`: `onCaseCreated`/`onCaseEndDateChanged`が`endDate`基準でタスクインスタンスを生成・再計算する(リネーム後回帰確認)
- **E2E (Playwright)**
