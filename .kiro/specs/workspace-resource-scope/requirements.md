...
ログインユーザーがどのワークスペースにも所属していない、または現在ワークスペースを選択していない場合は、対象リソースを扱う画面の一覧・作成に進めず、ワークスペース作成を促す空状態が表示される。

既存データは開発段階であり破棄前提でよい。クロスワークスペースでのデータ共有・移動、招待・RBAC、コメントや消化数ダッシュボードそのものの機能追加（集計ロジックの変更）は本仕様の対象外とする。ただし消化数画面を含む、ワークスペース所属リソースを扱う画面の未選択時空状態は本仕様の対象とする。
...
- In scope
  ...
  - タスク等から他リソースを参照するときの、同一ワークスペース整合性の検証（例: Taskの`caseId`／`parentTaskId`／`developmentStageId`）
  - タスクの担当者候補を現在ワークスペースのメンバーに限定すること
  - 繰り返しタスクテンプレートによるタスク自動生成が、テンプレートと生成先の案件とで同一ワークスペースの組み合わせに限られること
  - 上記5リソースまたはそれらを集計表示する画面（ホーム・案件・タスク・カンバン・カレンダー・繰り返し・非営業日・消化数）における、現在ワークスペース未選択時の空状態とワークスペース作成導線
- Out of scope
  - クロスワークスペースでのリソース検索・共有・移動
  - 招待リンク発行、ワークスペース内ロール／RBAC
  - コメント機能、操作ログ、消化ペース可視化（velocity-dashboard）そのものの機能追加（集計のワークスペーススコープ化を含む）
...
- Adjacent expectations
  ...
  - `/api/throughput`のワークスペーススコープ化は`velocity-dashboard`が担う。本仕様は同画面の未選択時空状態のみ先に入れる
...
#### Acceptance Criteria
1. If ログインユーザーがどのワークスペースにも所属していない、または現在ワークスペースを選択していない状態で、ワークスペース所属リソースを扱う画面（ホーム・案件・タスク・カンバン・カレンダー・繰り返しタスク・非営業日・消化数）にアクセスした場合、the Workspace Scope機構 shall 一覧や集計の内容を表示せず、ワークスペース作成を促す空状態を提示する
2. If ログインユーザーが現在ワークスペースを選択していない状態で当該画面からの新規作成や同期等の操作を試みた場合、the Workspace Scope機構 shall 操作を実行せず、ワークスペース作成を促す空状態を提示する
...
5. If ワークスペースのメンバーがリソース作成・更新時に、別ワークスペースに帰属するリソースIDを関連として指定した場合（例: Taskの`caseId`／`parentTaskId`／`developmentStageId`）、the Workspace Scope機構 shall その指定を拒否する
...
- 所属外 ID を指定した操作は拒否される（関連先 ID のクロスワークスペース指定を含む）
- 担当者選択の候補は、同一ワークスペースのメンバーに限られる
- 対象リソースを扱う画面と消化数画面は、未選択時にワークスペース作成へ誘導する
- 既存データは破棄前提でよい（開発段階）。マイグレーションはスキーマ適合を優先

### Approach

主要リソースに `workspaceId`（名称は design で確定）を追加し、list／get／update／delete を現在ワークスペース＋メンバーシップで強制する。作成時は現在ワークスペースに紐付ける。関連先リソース ID も同一ワークスペースであることを検証する。フロントの一覧・作成導線は現在ワークスペース前提に揃える。認証方式の詳細には依存せず、membership 判定 API／サービスを再利用する。
